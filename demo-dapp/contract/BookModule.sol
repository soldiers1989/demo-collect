 ...
 *            DemoModule本身也是一个合约，它需要部署到链上；同时，它又负责管理用户的合约。只有添加到模块中的用户合约，用户才能在dapp中调用这些合约
 */
...
contract BookModule is BaseModule {
    ...
    function BookModule() {
        ...
        //定义模块合约名称
        string memory moduleName = "BookModule";

        //定义模块合约名称
        string memory moduleDesc = "图书模块200";
        ...
        string memory moduleDescription = moduleName.concat(moduleVersion, "-Book-DAPP");

        //把合约注册到JUICE链上
        LibLog.log("register BookModule");
        ...
        LibLog.log("publish BookModule , exec insert ...");
        ret = addModule(tmpModule.toJson());
        moduleId = ret.recoveryToString();

        //添加用户合约到模块合约中
        LibLog.log("add BookManager to BookModule");
        ret = initContract(moduleName, moduleVersion, "BookManager", "图书合约", "0.0.1.0");
        if (ret != 0) {
            LibLog.log("add BookManager to BookModule failed");
            return;
        }


        //返回消息，以便控制台能看到是否部署成功
        Notify(0, "deploy BookModule success");
    }
